...
from Enums import Shapes, Colours


class QuantifierMetadataGen(MetadataGen):
    ...
    def gen_dataset(self):
        ...
        for row in self.df.itertuples():
            ...
            train = row.split
            case = row.case
            case = (Relationship(case[0]), Shapes(case[1]), Colours(case[2]), case[3])
            caption_abstract = row.caption_abstract
            gen = QuantifierConditionGen(
                    objects, case, self.settings, self.image_gen, train)
