...
class SpatialDatasetGen:
    ...
    def gen_dataset(self, numConditions):
        ...
        for i in range(numConditions):
            object_list = self.gen_objects(self.settings.num_objs)
            condition = SpatialConditionGen(object_list, i%2, self.settings, image_gen).gen_condition(
                num_tokens_per_condition)
    ...
    def gen_random_object(self):
        colours = [Colours.RED, Colours.BLUE, Colours.YELLOW]
        shapes = [Shapes.CIRCLE, Shapes.SQUARE, Shapes.STAR, Shapes.TRIANGLE]
